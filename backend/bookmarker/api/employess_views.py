...
@api_view(['GET'])
@permission_classes([IsAuthenticated]) # works for both html and json (no need for login_requried)
@renderer_classes([TemplateHTMLRenderer, JSONRenderer])
def emp_index(request):
    """
    index view for employees.
    """
    if not request.user.groups.filter(name='Employees').exists():
        return Response({'message': "You are not an employee."})
    
    if request.accepted_renderer.format == 'html':
        return Response(template_name='bookmarker/employees/index.html')
